...
func ensureAdminCredential(ctx context.Context, cfg config.Config, st *store.Store) (bool, string, error) {
	if existing, ok, err := st.LoadAdminCredential(ctx); err != nil {
		...
		if security.VerifyAdminKey(existing, cfg.AdminKey) {
			if !security.AdminCredentialNeedsUpgrade(existing) {
				return false, "", nil
			}
			credential, err := security.NewAdminCredential(cfg.AdminKey, existing.Source)
			if err != nil {
				return false, "", err
			}
