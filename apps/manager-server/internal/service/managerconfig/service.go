...
import (
	...
	"github.com/seakee/cpa-statistics/apps/manager-server/internal/service/cpa"
...
func (s *Service) Update(ctx context.Context, submitted store.ManagerConfig) (Response, error) {
	...
	if next.ExternalUsageService.Enabled && next.ExternalUsageService.ServiceBase == "" {
		return Response{}, errors.New("externalUsageService.serviceBase is required when externalUsageService is enabled")
	}
...
func (s *Service) MergeSubmittedManagerConfig(base store.ManagerConfig, submitted store.ManagerConfig) store.ManagerConfig {
	...
	submittedExternalServiceBase := cpa.NormalizeBaseURL(submitted.ExternalUsageService.ServiceBase)
	if submitted.ExternalUsageServiceSubmitted ||
		submitted.ExternalUsageService.Enabled ||
		submittedExternalServiceBase != "" {
		next.ExternalUsageService.Enabled = submitted.ExternalUsageService.Enabled
		next.ExternalUsageService.ServiceBase = submittedExternalServiceBase
	}
