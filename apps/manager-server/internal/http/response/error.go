...
func SetupErrorStatus(err error) int {
	...
	case strings.Contains(message, "cpaBaseUrl and managementKey are required"),
		strings.Contains(message, "externalUsageService.serviceBase is required"),
...
func UsageServiceErrorCode(err error) string {
	...
	case strings.Contains(message, "externalUsageService.serviceBase is required"):
		return "external_usage_service_base_required"
