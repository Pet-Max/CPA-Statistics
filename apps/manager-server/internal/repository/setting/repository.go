...
func (r *repository) protectSetup(setup model.Setup) (model.Setup, error) {
	urlDigest, err := security.NewConfigValueDigest(setup.CPAUpstreamURL)
	if err != nil {
		return model.Setup{}, err
	}
	managementKeyDigest, err := security.NewConfigValueDigest(setup.ManagementKey)
	if err != nil {
		return model.Setup{}, err
	}
	setup.CPAUpstreamURLDigest = urlDigest
	setup.ManagementKeyDigest = managementKeyDigest
	if r.protector == nil {
		return setup, nil
	}
	...
	value, err = r.protector.ProtectString(setup.CPAUpstreamURL)
	if err != nil {
		return model.Setup{}, err
	}
	setup.CPAUpstreamURL = value
	return setup, nil
}
...
func (r *repository) unprotectSetup(setup model.Setup) (model.Setup, error) {
	...
	setup.ManagementKey = value
	value, err = r.protector.UnprotectString(setup.CPAUpstreamURL)
	if err != nil {
		return model.Setup{}, err
	}
	setup.CPAUpstreamURL = value
...
func (r *repository) protectManagerConfig(cfg model.ManagerConfig) (model.ManagerConfig, error) {
	urlDigest, err := security.NewConfigValueDigest(cfg.CPAConnection.CPABaseURL)
	if err != nil {
		return model.ManagerConfig{}, err
	}
	managementKeyDigest, err := security.NewConfigValueDigest(cfg.CPAConnection.ManagementKey)
	if err != nil {
		return model.ManagerConfig{}, err
	}
	cfg.CPAConnection.CPABaseURLDigest = urlDigest
	cfg.CPAConnection.ManagementKeyDigest = managementKeyDigest
	if r.protector == nil {
		return cfg, nil
	}
	...
	value, err = r.protector.ProtectString(cfg.CPAConnection.CPABaseURL)
	if err != nil {
		return model.ManagerConfig{}, err
	}
	cfg.CPAConnection.CPABaseURL = value
	return cfg, nil
}
...
func (r *repository) unprotectManagerConfig(cfg model.ManagerConfig) (model.ManagerConfig, error) {
	...
	cfg.CPAConnection.ManagementKey = value
	value, err = r.protector.UnprotectString(cfg.CPAConnection.CPABaseURL)
	if err != nil {
		return model.ManagerConfig{}, err
	}
	cfg.CPAConnection.CPABaseURL = value
