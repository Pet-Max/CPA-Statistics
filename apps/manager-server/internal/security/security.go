...
const strongHashAlgorithm = "pbkdf2-sha256"
const adminHashIterations = 100_000
const configDigestIterations = 100_000
...
func AdminCredentialNeedsUpgrade(credential model.AdminCredential) bool {
	return credential.Algorithm != strongHashAlgorithm || credential.Iterations < adminHashIterations
}
...
func NewConfigValueDigest(value string) (*model.SecretDigest, error) {
	...
	hash, err := hashSecretValue(value, salt, configDigestIterations)
	if err != nil {
		return nil, err
	}
	return &model.SecretDigest{
		Version:     1,
		Algorithm:   strongHashAlgorithm,
		Salt:        base64.RawStdEncoding.EncodeToString(salt),
		Hash:        base64.RawStdEncoding.EncodeToString(hash),
		Iterations:  configDigestIterations,
		CreatedAtMS: time.Now().UnixMilli(),
	}, nil
}

func VerifyConfigValueDigest(digest *model.SecretDigest, value string) bool {
	value = strings.TrimSpace(value)
	if digest == nil || value == "" || digest.Salt == "" || digest.Hash == "" {
		return false
	}
	salt, err := base64.RawStdEncoding.DecodeString(digest.Salt)
	if err != nil {
		return false
	}
	want, err := base64.RawStdEncoding.DecodeString(digest.Hash)
	if err != nil {
		return false
	}
	...
	got, err := hashSecretValue(value, salt, iterations)
	if err != nil {
		return false
	}
	return subtle.ConstantTimeCompare(got, want) == 1
...
func NewProtector(key []byte) (*Protector, error) {
	...
	derived, err := hkdf.Key(sha256.New, key, []byte("cpa-statistics"), "settings-secrets-v1", 32)
	if err != nil {
		return nil, err
	}
...
func hashAdminKey(adminKey string, salt []byte, iterations int) ([]byte, error) {
	...
	return hashSecretValue(adminKey, salt, iterations)
}

func hashSecretValue(value string, salt []byte, iterations int) ([]byte, error) {
	if iterations <= 0 {
		return nil, errors.New("hash iterations must be greater than 0")
	}
	return pbkdf2.Key(sha256.New, value, salt, iterations, 32)
}
...
func deriveDataKey(value string) ([]byte, error) {
	...
	return hkdf.Key(sha256.New, []byte(value), []byte("cpa-statistics"), "data-key-v1", 32)
}
