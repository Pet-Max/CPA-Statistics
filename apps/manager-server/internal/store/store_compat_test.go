...
func TestStoreCompatSettingsUsageAndExport(t *testing.T) {
	...
	if loadedSetup.CPAUpstreamURL != setup.CPAUpstreamURL ||
		loadedSetup.ManagementKey != setup.ManagementKey ||
		loadedSetup.Queue != setup.Queue ||
		loadedSetup.PopSide != setup.PopSide {
...
func TestInsertEventsSanitizesImportedFailSummary(t *testing.T) {
	...
	event.FailSummary = "Authorization: Bearer demo-token api_key=fake-test-api-key"
	if _, err := db.InsertEvents(context.Background(), []usage.Event{event}); err != nil {
		t.Fatalf("insert event: %v", err)
	}
	...
	if strings.Contains(events[0].FailSummary, "demo-token") ||
		strings.Contains(events[0].FailSummary, "fake-test-api-key") ||
