...
func TestStoreEncryptsSetupAndManagerConfigSecrets(t *testing.T) {
	...
	if strings.Contains(rawSetup, "management-key") ||
		strings.Contains(rawSetup, "http://cpa.local:8317") ||
		!strings.Contains(rawSetup, "enc:v1:") {
	...
	if !security.VerifyConfigValueDigest(loadedSetup.CPAUpstreamURLDigest, "http://cpa.local:8317") ||
		!security.VerifyConfigValueDigest(loadedSetup.ManagementKeyDigest, "management-key") {
		t.Fatalf("loaded setup digests did not verify: %#v", loadedSetup)
	}
	...
	if strings.Contains(rawManagerConfig, "management-key") ||
		strings.Contains(rawManagerConfig, "http://cpa.local:8317") ||
	...
	if !security.VerifyConfigValueDigest(loadedManagerCfg.CPAConnection.CPABaseURLDigest, "http://cpa.local:8317") ||
		!security.VerifyConfigValueDigest(loadedManagerCfg.CPAConnection.ManagementKeyDigest, "management-key") {
		t.Fatalf("loaded manager config digests did not verify: %#v", loadedManagerCfg.CPAConnection)
	}
}
