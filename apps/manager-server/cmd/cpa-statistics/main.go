...
import (
	...
	"github.com/seakee/cpa-statistics/apps/manager-server/internal/command/adminreset"
	"github.com/seakee/cpa-statistics/apps/manager-server/internal/config"
	"github.com/seakee/cpa-statistics/apps/manager-server/internal/httpapi"
	"github.com/seakee/cpa-statistics/apps/manager-server/internal/security"
	bootstrapservice "github.com/seakee/cpa-statistics/apps/manager-server/internal/service/bootstrap"
	collectorservice "github.com/seakee/cpa-statistics/apps/manager-server/internal/service/collector"
	"github.com/seakee/cpa-statistics/apps/manager-server/internal/store"
	"github.com/seakee/cpa-statistics/apps/manager-server/internal/worker"
)
...
func runServer() {
	...
	if bootstrapResult.GeneratedAdminKey != "" {
		log.Printf("CPA Statistics admin key generated: %s", bootstrapResult.GeneratedAdminKey)
	} else {
		log.Printf("CPA Statistics admin credential initialized")
	}
	if bootstrapResult.DataKeyCreated {
		log.Printf("CPA Statistics data key created at %s", cfg.DataKeyPath)
	}
	if bootstrapResult.MigratedLegacy {
		log.Printf("CPA Statistics legacy data migrated")
	}
	...
	go func() {
		log.Printf("cpa-statistics listening on %s", cfg.HTTPAddr)
