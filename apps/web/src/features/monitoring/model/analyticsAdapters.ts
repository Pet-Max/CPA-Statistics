...
export const buildTimelineFromAnalytics = (
  ...
  const maxRequests = points.reduce((max, point) => Math.max(max, point.calls), 0);
  ...
  return points.map((point) => ({
    ...
    requestsShare: maxRequests > 0 ? point.calls / maxRequests : 0,
    tokensShare: maxTokens > 0 ? point.tokens / maxTokens : 0,
  }));
};
