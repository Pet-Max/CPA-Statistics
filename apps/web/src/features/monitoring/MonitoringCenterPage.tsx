...
import { UsageInsightsPanel } from '@/features/monitoring/components/UsageInsightsPanel';
import {
  RealtimeEventsPanel,
...
type MonitoringCenterPageMode = 'usage' | 'details' | 'all';

interface MonitoringCenterPageProps {
  mode?: MonitoringCenterPageMode;
}

export function MonitoringCenterPage({ mode = 'all' }: MonitoringCenterPageProps) {
  ...
  const {
    ...
    modelShareRows: monitoringModelShareRows,
  ...
  const primarySummaryCards = useMemo(
    ...
    [accountRows.length, hasPrices, i18n.language, scopedSummary, t]
  );
