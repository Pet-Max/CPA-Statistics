...
export interface UseUsageDataReturn {
  ...
  clearData: (confirmation: ClearDataRequest['confirmation']) => Promise<ClearDataResponse>;
...
export function useUsageData({
  ...
  const clearDataFromApi = useCallback(async (confirmation: ClearDataRequest['confirmation']): Promise<ClearDataResponse> => {
    if (!usageEventsServiceBase) {
      throw new Error('usage_import_export_requires_usage_service');
    }
    return usageServiceApi.clearData(
      usageEventsServiceBase,
      {
        confirmation,
        scope: { usageEvents: true, deadLetters: true },
      },
      managementKey
    );
  }, [managementKey, usageEventsServiceBase]);
  ...
  const syncModelPrices = useCallback(
    async (models?: string[]) => {
      ...
      const syncedPrices = response.prices ?? {};
